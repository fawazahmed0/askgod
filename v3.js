...
async function getQueryVerses (query) {
  // This needs to be added somewhere else
  const [questionVerses] = await getLinksJSON([questionVerseLink])
...
// Takes query and gives the confirmed verses
async function begin(query){
  // needs to be placed somewhere else
  await getTranslations(translationLinks)

 let savedVerses = await getQueryVerses (query)

 if(savedVerses.length>0)
      return savedVerses

      try {
          // keep this somewhere to fix the issue, probably in initialize
saveToDB(query)
      } catch (error) {
   ...
   let googlinks = await getGoogleLinks (engQuery+' in quran')


    let htmlstr =  await linksFetcher (googlinks)

   let parsedString =  htmlToString(htmlstr)
   let confirmedVerses = await gestaltInference(parsedString)

   let sortedVerses = confirmedVerses.map(e=>e.split(',').map(e=>parseInt(e))).sort(chronologicalSort)
return sortedVerses
